...
const formatGraphError = (rawText: string, statusText: string) => {
  try {
...
const assertFetchAvailable = () => {
  if (typeof fetch !== "function") {
    throw new Error(
      "[Azure] fetch no esta disponible. Usa Node 18+ o agrega un polyfill."
    );
  }
};
...
export const getGraphToken = async () => {
  ...
  if (!response.ok || !data?.access_token) {
    console.error(
      ...
      buildLogMeta({ status: response.status }),
      { raw: rawText }
    );
...
const buildEmailHtml = (payload: ContactMailPayload, meta?: ContactMailMeta) => {
  const rows = [
  ...
  rows.push(
    `<p><b>Mensaje:</b></p><p>${escapeHtml(payload.mensaje).replace(/\n/g, "<br/>")}</p>`
  );

  rows.push(
    `<p><b>Fecha:</b> ${escapeHtml(
...
export const sendContactMail = async (
  ...
  const graphBase =
    getOptionalEnv("AZURE_GRAPH_BASE_URL") ?? DEFAULT_GRAPH_BASE_URL;
  ...
  const response = await fetch(
    `${graphBase}/users/${encodeURIComponent(sender)}/sendMail`,
    {
      method: "POST",
      headers: {
